...
public class CartController {
	...
	public ResponseEntity<Cart> addToCart(@PathVariable("vendorItemId") int vendorItemId,
	...
	public ResponseEntity<List<Cart>> viewCart(HttpServletRequest request) {
		return cartService.viewCartImpl(request);
	}

	@PreAuthorize("hasRole('ROLE_customer')")
	@DeleteMapping("/cart/{cartId}") // Delete items from cart for customer
	public ResponseEntity<Cart> deleteCart(@PathVariable("cartId") int cartId, HttpServletRequest request) {
